...
export function EnrollmentProcessor() {
  ...
  const processEnrolledApplication = async (application: any) => {
    try {
      ...
      // Determine email to use; even if it exists, we proceed idempotently
      ...
      console.log(`Using email ${potentialEmail} for ${application.application_number} (will reuse if existing)`);
