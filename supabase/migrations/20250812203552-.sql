-- Fix the create_complete_student_enrollment function to correctly handle student_parents relationship
...
BEGIN
  ...
  IF parent_email IS NOT NULL AND parent_email != '' THEN
    ...
    -- FIX: Use the actual students table ID, not user_id
    INSERT INTO student_parents (
      student_id, 
      parent_id, 
      relationship
    ) VALUES (
      new_student_record_id,  -- This is the students.id field (UUID from students table)
      new_parent_user_id,     -- This is the parent's user_id (profiles.user_id)
