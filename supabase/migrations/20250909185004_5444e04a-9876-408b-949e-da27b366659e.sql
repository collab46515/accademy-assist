-- Enforce single-school globally: block creating a second school
...
BEGIN
  ...
  -- Enforce single-school globally
  SELECT EXISTS (SELECT 1 FROM public.schools s WHERE COALESCE(s.is_active, true) = true) INTO any_school_exists;
  IF any_school_exists THEN
    RAISE EXCEPTION 'A school already exists in this system. Please contact an administrator.';
  END IF;
