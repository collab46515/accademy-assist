...
-- Update existing payment_method values to match constraint
UPDATE public.payment_records 
SET payment_method = 'cash' 
WHERE payment_method NOT IN ('cash', 'bank', 'online', 'cheque');
...
-- Add check constraint for payment_method after cleaning data
ALTER TABLE public.payment_records 
...
-- Generate receipt numbers for existing records that don't have them
UPDATE public.payment_records 
SET receipt_number = generate_receipt_number()
WHERE receipt_number IS NULL;

-- Add unique constraint to receipt_number after populating
ALTER TABLE public.payment_records 
ADD CONSTRAINT payment_records_receipt_number_unique 
